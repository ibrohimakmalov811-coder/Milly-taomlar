      ...
      >
        ...
        <div className="p-4 border-t" style={{ borderColor: "rgba(246,239,224,0.1)" }}>
          <div className="flex items-center gap-3 px-2 py-2 rounded-xl">
            <div className="w-9 h-9 rounded-full flex items-center justify-center font-display text-sm font-semibold shrink-0" style={{ background: "var(--gold-300)", color: "var(--brown)" }}>SN</div>
            <div className="flex-1 min-w-0">
              <p className="text-[13px] font-semibold font-body truncate" style={{ color: "var(--cream)" }}>Sardor Nazarov</p>
              <p className="text-[11px] font-body opacity-55 truncate">Bosh administrator</p>
            </div>
            <button title="Chiqish" className="w-8 h-8 rounded-lg flex items-center justify-center hover:bg-white/10 shrink-0">
...
function Header({ setMobileOpen, theme, setTheme, notifications, activeLabel }) {
    ...
    >
      ...
      <div className="flex items-center gap-1.5 ml-auto">
        ...
        <div className="flex items-center gap-2 pl-2 ml-1" style={{ borderLeft: "1px solid var(--line)" }}>
          <div className="w-8 h-8 rounded-full flex items-center justify-center font-display text-xs font-semibold" style={{ background: "var(--gold-300)", color: "var(--brown)" }}>SN</div>
          <span className="hidden sm:block text-xs font-semibold font-body" style={{ color: "var(--brown)" }}>Sardor N.</span>
        </div>
      </div>
    </header>
  );
...
function DashboardView({ orders, reservations, foods, chefs, theme }) {
  const popularFoods = [...foods].sort((a, b) => b.rating - a.rating).slice(0, 5);
  return (
    <div className="space-y-6">
      <SectionHeader title="Xush kelibsiz, Sardor" subtitle="Bugungi restoran faoliyati bo'yicha umumiy ko'rinish" />
    ...
    <div>
      ...
      <Card className="p-6 max-w-2xl">
        <form onSubmit={(e) => { e.preventDefault(); notify("Sozlamalar saqlandi"); }} className="space-y-4">
          ...
          {tab === "profile" && (
            <>
              <div className="w-16 h-16 rounded-full flex items-center justify-center font-display text-xl font-semibold mb-2" style={{ background: "var(--gold-300)", color: "var(--brown)" }}>SN</div>
              <Input label="To'liq ism" defaultValue="Sardor Nazarov" />
              <Input label="Email" defaultValue="sardor.nazarov@millytaomlar.uz" />
...
export default function App() {
  ...
  const views = {
    dashboard: <DashboardView orders={orders} reservations={reservations} foods={foods} chefs={chefs} theme={theme} />,
  ...
  return (
    <div className={theme === "dark" ? "dark" : ""}>
      ...
      <div className="flex min-h-screen font-body" style={{ background: "var(--cream)" }}>
        <Sidebar active={active} setActive={setActive} mobileOpen={mobileOpen} setMobileOpen={setMobileOpen} theme={theme} />
        <div className="flex-1 min-w-0 flex flex-col">
          <Header setMobileOpen={setMobileOpen} theme={theme} setTheme={setTheme} notifications={notifications} activeLabel={activeLabel} />
